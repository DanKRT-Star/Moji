    ...
    try {
        ...
        if (type === 'direct') {
            ...
            conversation = await Conversation.findOne({type: 'direct',
                "participants._id": {$all: [userId, participantId]}
            });

            if (!conversation) {
                conversation = new Conversation({
                    type: 'direct',
                    participants: [{_id: userId}, {_id: participantId}],
        ...
        await conversation.populate([
            {path: 'participants._id', select: 'displayName avatarUrl'},
...
export const markAsSeen = async (req, res) => {
    try {
        ...
        if (last.senderId.toString() === userId) {
            return res.status(200).json({message: "Bạn không thể đánh dấu tin nhắn của chính mình là đã xem"});
        }
        ...
        console.error("Lỗi xảy ra khi đánh dấu tin nhắn là đã xem", error);
        return res.status(500).json({message: "Lỗi hệ thống"})
    }
};