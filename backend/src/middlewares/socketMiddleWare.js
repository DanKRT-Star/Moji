...
export const socketAuthMiddleware = async (socket, next) => {
    try {
        ...
        const devoded = jwt.verify(token, process.env.ACCESS_TOKEN_SECRET);

        if (!devoded) {
            return next(new Error("Unauthorized - Token không hợp lệ hoặc đã hết hạn"));
        }

        const user = await User.findById(devoded.userId).select("-hashedPassword");
